...
- **Intro — "Engineering Orbit"** (`components/intro/Intro.tsx`): concept premium/minimal, bố cục theo mockup thiết kế = **1 cột identity giữa màn hình**, orbit **quầng quanh avatar**. Avatar = **core** (glow khí quyển, không viền nặng); rings + nodes + particle nằm ở **lớp absolute căn tâm theo avatar** (scale + parallax), **tràn ra ngoài** avatar. Tên **ngay dưới avatar** (uppercase "VO NHU" trắng + "PHI" gradient cyan→blue), rồi role mono cyan letter-spaced, tagline, stats premium (03+/30K+/04M+), scroll indicator. Tech xếp trên **2 quỹ đạo ellipse** (`RINGS`): ring trong = application (Frontend+Backend), ring ngoài = platform (Data+Infra); màu = category, ring = tầng kiến trúc. `START/SPAN` chừa **khoảng hở bottom-center** cho tên; **scrim radial** sau khối tên để tên luôn đọc rõ trên node gần. **RESPONSIVE**: toàn bộ composition (orbit + text) được bọc trong 1 wrapper **scale đồng nhất** = `min(1, w/1080, h/820)` (bound cả width lẫn height) nên trên laptop nhỏ / **zoom 150%** mọi thứ co lại vừa khung, KHÔNG bị tràn/đè. Wrapper `items-center` + `pb-16` chừa chỗ cho scroll indicator; scroll cue ẩn khi `height < 600px`. Node hover → sáng + tooltip category + nổi ring liên quan. Particle SMIL `animateMotion`. **Mobile** (`compact`, `<680px`) = layout **flex dọc tự nhiên** (avatar → tên → role → tagline → chip tech gọn → stats), KHÔNG dùng composition absolute. Tôn trọng `prefers-reduced-motion`. **Skip** khi `?static`/`#hash`.
- **Hero** (`sections/Hero.tsx` + `sections/HeroGraphic.tsx`): visual chính bên phải = **terminal "whoami"** (`HeroGraphic`) — mac-window in ra 1 session cá nhân/vui: `whoami` → tên+role, `cat traits.json` → JSON có humour (`coffee_level: "dangerously high ☕"`, `dark_mode: true`), `./ship --prod` → `✓ 4M+ orders/month · still sleeps fine 😴`, kết bằng con trỏ nhấp nháy. Lines reveal so-le (staggerChildren), token tô màu bằng span, localize vài string qua `pick()`. Pure HTML/CSS → luôn hiện dù WebGL tắt. **RESPONSIVE**: terminal chỉ hiện `lg:block` (dưới đó hero chỉ còn text, tránh card chật/tràn trên tablet); font code `clamp(9.5px,0.78vw,12.5px)` để không bị cắt phải; content bên trái chừa nửa phải `lg:pr-[50%]` để chữ KHÔNG chui dưới terminal. WebGL scene (`three/HeroScene.tsx`) chỉ còn **ParticleCloud** ambient. Tagline có **thanh accent trái** (quote style).
