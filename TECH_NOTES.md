...
- **Global type/spacing scale** (`index.css`): `html { font-size: 16px }`, và `@media (max-width:1535px) { html { font-size: 15px } }`. Tailwind dùng rem nên hạ root font ~6% trên viewport ≤1535 (14" hi-dpi, zoom 150%) → mọi heading/chip/gap co lại đều, section hết cảm giác "ngộp". Màn ≥1536 giữ full. `SectionHeading` title cap `text-4xl` tới `2xl:text-5xl` (chỉ to nhất trên màn lớn).

- **Intro — "Engineering Orbit"** (`components/intro/Intro.tsx`): concept premium/minimal, bố cục theo mockup thiết kế = **1 cột identity giữa màn hình**, orbit **quầng quanh avatar**. Avatar = **core** (glow khí quyển, không viền nặng); rings + nodes + particle nằm ở **lớp absolute căn tâm theo avatar** (scale + parallax), **tràn ra ngoài** avatar. Tên **ngay dưới avatar** (uppercase "VO NHU" trắng + "PHI" gradient cyan→blue), rồi role mono cyan letter-spaced, tagline, stats premium (03+/30K+/04M+), scroll indicator. Tech xếp trên **2 quỹ đạo ellipse** (`RINGS`): ring trong = application (Frontend+Backend), ring ngoài = platform (Data+Infra); màu = category, ring = tầng kiến trúc. `START/SPAN` chừa **khoảng hở bottom-center** cho tên; **scrim radial** sau khối tên để tên luôn đọc rõ trên node gần. **RESPONSIVE**: toàn bộ composition (orbit + text + **scroll cue**) bọc trong 1 wrapper **scale đồng nhất** = `min(1, w/1080, h/760)` (bound cả width lẫn height) → laptop nhỏ / **zoom 150%** mọi thứ co vừa khung, KHÔNG tràn/đè. **Scroll cue nằm IN-FLOW** (cuối column, `ScrollCue`) — KHÔNG absolute/không gate theo height nữa (trước đây `@media(min-height:600px)` làm mất cue trên 14"); giờ luôn hiện, scale cùng composition. Node hover → sáng + tooltip category + nổi ring liên quan. Particle SMIL `animateMotion`. **Mobile** (`compact`, `<680px`) = layout **flex dọc tự nhiên** (avatar → tên → role → tagline → chip tech gọn → stats), KHÔNG dùng composition absolute. Tôn trọng `prefers-reduced-motion`. **Skip** khi `?static`/`#hash`.
- **Hero** (`sections/Hero.tsx` + `sections/HeroGraphic.tsx`): visual chính bên phải = **terminal "whoami"** (`HeroGraphic`) — mac-window in ra 1 session cá nhân/vui: `whoami` → tên+role, `cat traits.json` → JSON có humour (`coffee_level: "dangerously high ☕"`, `dark_mode: true`), `./ship --prod` → `✓ 4M+ orders/month · still sleeps fine 😴`, kết bằng con trỏ nhấp nháy. Lines reveal so-le (staggerChildren), token tô màu bằng span, localize vài string qua `pick()`. Pure HTML/CSS → luôn hiện dù WebGL tắt. **RESPONSIVE**: hero content = **grid 2 cột** (`lg:grid-cols-2`, text trái + terminal phải) trong `container-page` → hai cột **luôn thẳng hàng**, không đè (thay cho cách cũ absolute + `lg:pr-[50%]` bị vỡ trên màn thường). Terminal chỉ hiện `lg:block` (dưới lg hero chỉ còn text 1 cột); tên `text-5xl sm:text-6xl 2xl:text-7xl` (chỉ to nhất trên màn lớn); font code `clamp(9.5px,0.78vw,12.5px)`. WebGL scene (`three/HeroScene.tsx`) chỉ còn **ParticleCloud** ambient. Tagline có **thanh accent trái** (quote style).
- **Skills** (`sections/Skills.tsx`): **tab tầng NGANG** + panel **breakdown phân hạng mục màu-coded** cho tầng đang chọn (KHÔNG còn sơ đồ kiến trúc tổng; sơ đồ thật vẫn ở `GatewayDeepDive`/Kafka/Saga). Data: `skillGroups[].categories[]` = `{ label{en,vi}, items[] }` + `blurb{en,vi}` mỗi group. Mỗi category = 1 card `sm:grid-cols-2` với **màu + icon riêng** (`CAT_META` keyed theo `label.en`: Language cyan/Code2, Frameworks emerald/Layers, API&RPC violet/Webhook, Architecture blue/Boxes, Messaging amber, Patterns pink, …), có **icon-tile gradient**, heading màu, **watermark góc** mờ; chip = logo thật (`hasLogo`) hoặc **dot màu** fallback. Panel header = icon group + title + blurb + badge "N Skills". Dưới heading có subtitle (`ui.skills.sub`); góc phải heading có **cube isometric** (`CubeGlyph`). Cuối section = **strip soft-skills 4 ô** (Always Learning / Problem Solver / Clean Code / Team Player). Đổi tab → crossfade. **Layout card = flexbox căn giữa, số cột theo số category** (`n<=3 → n`, `4 → 2`, `5–6 → 3`; `flexBasis` tính theo cols) nên hàng cuối luôn **căn giữa, không có card lẻ rớt xuống**; mobile ép 1 cột.
- **About** (`sections/About.tsx`): khoảng cách đã nén (`mt-8/gap-8`, stats `mt-7`, cards `mt-6`) để section gọn trong khung hình.
