...
- **Intro — "Engineering Orbit"** (`components/intro/Intro.tsx`): concept premium/minimal. Avatar = **core** (glow khí quyển, không viền nặng). Tech xếp trên **2 quỹ đạo ellipse** (`RINGS`): ring trong = application (Frontend+Backend), ring ngoài = platform (Data+Infra); màu = category, ring = tầng kiến trúc. Node **nhỏ, viền/nền rất nhẹ**, hover mới sáng + hiện tooltip category + làm nổi ring liên quan (`hover` state). Vài particle chạy chậm dọc ellipse (SMIL `animateMotion`). Parallax con trỏ nhẹ (desktop). Chữ: **tên là focal chính** (Vo Nhu trắng, Phi gradient cyan→blue), role mono cyan, tagline, stats premium (03+/30K+/04M+). Indicator = **"Scroll to explore" + chevron xuống** (bỏ "pull up to enter" kiểu game). `stageTop` nâng orbit lên khi viewport thấp (`short`) để **tên không đè avatar**. **Mobile** (`compact`, `<680px`) = layout **flex dọc tự nhiên** (avatar → tên → role → tagline → chip tech gọn → stats), KHÔNG dùng composition absolute (tránh chồng/tràn ngang). Tôn trọng `prefers-reduced-motion`. **Skip** khi `?static`/`#hash`.
...
- **Skills** (`sections/Skills.tsx`): desktop = **tab tầng NGANG ở trên** + **sơ đồ** bên dưới, card **cap chiều cao** (`maxWidth: min(100%, 58vh*900/560)`, `mx-auto`) để **tab + sơ đồ nằm chung 1 khung hình** — đổi tầng không phải cuộn. Zone đang chọn có **khung dashed highlight** (BAND). Nhãn cạnh to, có viền chữ. **Hover node → tooltip** 300px phân loại stack (`NODE_DETAIL`); glow bọc lớp riêng nên tooltip không bị cắt. Payment qua Temporal task-queue (async), **không gRPC trực tiếp**. Mobile = `ArchStack`.
