...
- **Hero** (`sections/Hero.tsx` + `sections/HeroGraphic.tsx`): visual chính bên phải = **SVG "microservices mesh"** (`HeroGraphic`) — hub API Gateway + 6 node (React/Go/Kafka/Postgres/NestJS/Redis), link + packet chạy. **Luôn hiển thị** kể cả khi WebGL tắt/degraded (khắc phục "không có ảnh"). WebGL scene (`three/HeroScene.tsx`) giờ chỉ còn **ParticleCloud** ambient (đã bỏ wireframe globe). Vignette làm nhẹ + lệch trái để không che graphic.
- **Skills** (`sections/Skills.tsx`): **tab tầng NGANG ở trên** + panel **breakdown phân hạng mục** cho tầng đang chọn (KHÔNG còn sơ đồ kiến trúc tổng — bị coi là thừa; sơ đồ thật vẫn ở `GatewayDeepDive`/Kafka/Saga). Data: `skillGroups[].categories[]` = `{ label{en,vi}, items[] }` (vd Backend: Language / Frameworks / API & RPC / Architecture / Messaging / Patterns). Mỗi category = 1 card `sm:grid-cols-2`, label mono + chip có logo (`hasLogo`/`TechGlyph`). Badge số trên tab = tổng items (`totalItems`). Đổi tab → crossfade (`AnimatePresence`). Cả section gọn trong 1 khung hình. Cùng UI cho mọi kích thước (tab wrap trên mobile).
