...
export function DeepDiveLayout({ id, kicker, title, body, children, flip, wide, legend }: Props) {
  return (
        ...
        <div className={`grid items-center gap-10 lg:grid-cols-2 ${flip ? 'lg:[&>*:first-child]:order-2' : ''}`}>
          <Reveal>
            <div className="flex flex-col gap-4">
              <span className="chip w-fit font-mono uppercase tracking-[0.2em] text-accent-violet">
                <span className="h-1.5 w-1.5 rounded-full bg-accent-violet shadow-glow-violet" />
                {kicker}
              </span>
